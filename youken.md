...
- **通信方式**: REST API（Pull型）、HTTP（開発テスト環境用）
- **APIエンドポイント**: GET http://localhost:5001/api/v1/inventory
- **認証方式**: なし（開発テスト環境用）
- **データ形式**: JSON
- **データ種別**: 全量データ
...
- **通信方式**: REST API、HTTP（開発テスト環境用）
- **APIエンドポイント**: POST http://localhost:5001/api/v1/shipment-requests
- **認証方式**: なし（開発テスト環境用）
- **データ形式**: JSON
...
- **通信方式**: REST API（LC側から本システムのエンドポイントを呼び出し）、HTTP（開発テスト環境用）
- **APIエンドポイント**: POST http://localhost:8000/api/v1/shipment-results
- **認証方式**: なし（開発テスト環境用）
- **データ形式**: JSON
...
- **注意**: 開発テスト環境では認証なし・HTTP通信のため、本番環境へのデプロイ前にHTTPS・JWT認証を実装すること
...
- **通信プロトコル**: HTTP（開発テスト環境用。本番環境ではHTTPS（TLS 1.2以上）を使用予定）
- **開発環境**: 
...
7. **認証方式**: 開発テスト環境では認証なし（本番環境ではJWT認証を実装予定）
8. **通信プロトコル**: 開発テスト環境ではHTTP（本番環境ではHTTPS（TLS 1.2以上）を使用予定）
